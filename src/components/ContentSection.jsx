...
export default function ContentSection({ title, subtitle, features, image, imageAlt, builtForAI, reverse }) {
  return (
    <section className="bg-contentBg py-4">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-cardBg rounded-lg shadow-lg p-8 border border-gray-800 h-[375px] flex flex-col lg:flex-row items-center">
          {/* Conditional rendering based on the reverse prop */}
          {!reverse ? (
            <>
              <div
                ...
                <ul className="space-y-3">
                  {features.map((feature, index) => (
                    <li key={index} className="flex items-center">
                      <span className="bg-gray-600 rounded-full w-4 h-4 flex items-center justify-center mr-2">
            ...
            <>
              ...
              <div
                className="lg:w-1/2 mb-8 lg:mb-0 h-full overflow-y-auto pr-4 ml-6"
                initial={{ opacity: 0, x: 50 }}
                whileInView={{ opacity: 1, x: 0 }}
                transition={{ duration: 0.8 }}
                viewport={{ once: true }}
